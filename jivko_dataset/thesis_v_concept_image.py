...
shape_im = ['9_cluster', '3_cluster', '1_cluster', '8_cluster']
color_im = ['2_cluster', '3_cluster', '5_cluster', '7_cluster']
...
for c1,i in enumerate(all_images):
    ...
    for c2,j in enumerate(i):
        ...
        print dir_+j+".jpg"
        image[c1*(im_len*5)+c1*th:(c1+1)*(im_len*5)+c1*th, c2*(im_len*5)+c2*th:(c2+1)*(im_len*5)+c2*th, :] = im
        cv2.putText(image,t+'_'+str(c2),   (100+c2*(im_len*5)+c2*th, (c1+1)*(im_len*5)+(c1)*th+th*1/2),    font, .8,(0,0,0),2)
        # print t
