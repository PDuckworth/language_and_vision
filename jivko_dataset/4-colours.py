...
class colours():
    ...
    def __init__(self):
        ...
        self.dir_save = "/home/omari/Datasets/jivko_dataset/features/colours/"
        self.th = 10
        self.sp = 2
        self.X = []     # fpfh vales
        self.eX = []    # esf vales
    ...
    def _read_colours(self):
        for t in range(1,5):
            for o in range(1,33):
                f2 = self.dir+"obj_"+str(t)+"_"+str(o)+"_colour.txt"
                im = self.dir+"obj_"+str(t)+"_"+str(o)+".jpg"
                img = cv2.imread(im)
                s = img.shape

                A = img[s[0]/2-20:s[0]/2+20, s[1]/2-20:s[1]/2+20, :]
                bgr = [np.sum(A[:,:,0])/(1600), np.sum(A[:,:,1])/(1600), np.sum(A[:,:,2])/(1600)]
                img = np.zeros((self.im_len,self.im_len,3),dtype=np.uint8)+bgr
                # cv2.imshow("test",img)
                # cv2.waitKey(500)
                cv2.imwrite(self.dir+"obj_"+str(t)+"_"+str(o)+"_colour.jpg",img)
                ...
                for line in f:
                    ...
                    if line not in self.shapes:
                        self.shapes[line] = [bgr]
                    else:
                        self.shapes[line].append(bgr)
                f.close()
