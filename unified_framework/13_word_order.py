...
from Word_order_functions import *
...
object_number_max = 2		# maximum number of objects allowed in one scene

display_frame = 30		# number of examples to update the robot brain
rotation_inc = 300		# rotation increament for the robot brain view
...
if __name__ == "__main__":
	...
	f_order=plt.figure(2)
	f_order.suptitle('Word order graph', fontsize=20)
	...
	#========================== MAIN LOOP ====================================#
		...
		for i in range(n_examples):
			...
			# Learning start here:
			hyp = sentence_parsing(hyp,sentence)			# create a set of words and increament the counter
			print '-----------------------------------'
...
#===========##============##===========$$============##===========##==============#	WITH CK
			...
			if (i+1)%display_frame == 0:
				...
				#Plotting(POINTS_HSV,ax_HSV,rotation_inc,4,25,'HSV',i)

				#Plotting_SPA_hypotheses(hyp,ax_SPA)
				Plotting_dis_hypotheses(hyp,ax_DIS)
				Plotting(POINTS_SPA,ax_DIS,rotation_inc,5,85,'Distance',i)

				Plotting_dir_hypotheses(hyp,ax_DIR)
				Plotting(POINTS_SPA,ax_DIR,rotation_inc,6,85,'Direction',i)
...
#===========##============##===========$$============##===========##==============#	WORD order
			...
			hyp = word_order(hyp,sentence)				# for word order

			#word_order2(sentence)
			#hyp = word_order_consistency(hyp)			# check for consistency between words
			make_order_graph(hyp,f_order)				# update the word order graph


			#cv2.imwrite(Dir+'Scene'+'-'+str(i+1)+'.png',img2)
			cv2.imshow('Table',img)
			k = cv2.waitKey(time) & 0xFF
