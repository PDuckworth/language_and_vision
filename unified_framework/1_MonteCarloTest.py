...
if __name__ == "__main__":
	...
	for object_number_max in range(object_max):
	  RESULTS[object_number_max] = {}
	  for color_number_max in range(1,color_max+1):
	    RESULTS[object_number_max][color_number_max] = 0
	...
	#========================== MAIN LOOP ====================================#
		...
		if A == color_number_max:
			...
			RESULTS[object_number_max][color_number_max] += 1
		else:
	...
	for c, z in zip(['r', 'g', 'b', 'y'], np.arange(0,object_max)):
	    xs = np.arange(1,color_max+1)
	    ys = []
	    for j in xs:
		ys.append(RESULTS[z][j])
