...
if __name__ == "__main__":
	...
	#plt.ion()
	#plt.show()
	f4.suptitle('Feature Space', fontsize=20)
	ax4.set_xlabel('X')
	ax4.set_ylabel('Y')
	ax4.set_zlabel('Z')
		...
		for i in range(n_examples):
			...
			for j in sentence[i].split(' '):
				if j not in words:
					...
					hyp[j]['hist3'] = np.zeros(shape=(201,201,101))
					#hyp[j]['score'] = np.zeros(360)
					hyp[j]['score3'] = np.zeros(shape=(201,201,101))
			...
			#------------------ Plot HSV/compute HSV hypothesis ---------------------#
			...
			for j in words2:
				...
				for p in range(len(H_value)):
					...
					#ax3.scatter(xs, ys, zs, c=(c[0]/255.0,c[1]/255.0,c[2]/255.0), marker='o')
			...
			if (i+1)%display_frame == 0:
				for j in words:
					#------------------ Compute Hypotheses -------------#
					X,Y,Z = np.nonzero(hyp[j]['hist3'])	# loop through the histogram values
					flag = 0
					for kk in [8,12,15,18,20,23,25]:
						...
						if  np.sum(hyp[j]['hist3'][x1:x2,y1:y2,z1:z2]) >= hyp[j]['counter']*.9:
							hyp[j]['score3'][x1:x2,y1:y2,z1:z2] = 1
							flag = 1
			...
			#---------- find which words has hypotheses ----------#
				plt.figure(4)
				plt.cla()

				counter = 0
				for j in words:
					A = np.nonzero(hyp[j]['score3'])
					if A[0] != []:
						#print j
				...
				#ax3.set_ylim([-1,1])
				#ax3.set_zlim([0,1])
				for kk in range(0,180,5):
					plt.figure(4)
					ax4.view_init(elev=25, azim=kk)
					#plt.figure(3)
					#ax3.view_init(elev=25, azim=kk)
				   	plt.draw()
