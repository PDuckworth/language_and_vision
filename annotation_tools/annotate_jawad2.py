...
class annotation():
    ...
    def __init__(self):
        ...
        self.to_save = []
        self.saved = []
        self.x0,self.x1,self.y0,self.y1 = 0,800,0,500
        self.dir1 = "/home/omari/Downloads/vid32.csv"
        self.dir2 = "/home/omari/Downloads/vid42_b.csv"
        self._read_data()
        self._annotate()
    ...
    def _read_data(self):
        ...
        self.img_names = np.zeros((len(self.data)*20,300,3),dtype=np.uint8)+240
        ...
        self.img = np.concatenate((self.img_names[0:self.y1,:,:], self.img_data[0:self.y1,0:self.x1,:]), axis=1)
        # self.img = np.concatenate((self.img, self.img_control), axis=0)
        cv2.namedWindow('image')
        cv2.setMouseCallback('image',self.draw_circle)

    def _control(self):
        self.img_control = np.zeros((300,300,3),dtype=np.uint8)+250
        self.up = [(110,50),(250,100)]
        self.lf = [(10,120),(150,170)]
        self.dw = [(110,190),(250,240)]
        self.ri = [(210,120),(350,170)]
        self.nx = [(40,260),(300,330)]
        cv2.rectangle(self.img_control,self.up[0],self.up[1],(0,155,0),-1)
        cv2.rectangle(self.img_control,self.dw[0],self.dw[1],(155,0,0),-1)
        cv2.rectangle(self.img_control,self.ri[0],self.ri[1],(0,0,155),-1)
        cv2.rectangle(self.img_control,self.lf[0],self.lf[1],(155,155,0),-1)
        cv2.rectangle(self.img_control,self.nx[0],self.nx[1],(155,0,155),-1)
    ...
    def draw_circle(self,event,x,y,flags,param):
        if event == cv2.EVENT_LBUTTONDOWN:
            c = int(y/20)+self.y0/20
            l = self.labels[c]
            if str(c) not in self.saved:
    ...
    def draw_circle2(self,event,x,y,flags,param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.inside([x,y],self.nx):
                ...
                font = cv2.FONT_HERSHEY_SIMPLEX
                for c,l in enumerate(self.labels):
                    if str(c) in self.saved:
                        self.img_data[c*20:(c+1)*20,self.data[l][0]:self.data[l][1],:] = (200,200,200)
