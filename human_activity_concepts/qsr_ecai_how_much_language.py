...
def get_sentance_annotation(vid):
    ...
    """
    annotate_dir = "/home/paul/Datasets/ECAI_Data/dataset_activity_and_person_annotations/" + vid
...
if __name__ == "__main__":
    ...
    dynamic_args = { "qtcbs": {"qsrs_for" : qsrs_for, "no_collapse": True, "quantisation_factor":0.1, "validate":False },
                     ...
                     "qstag" : {"params" : {"min_rows": 1, "max_rows": 1, "max_eps": 3, "ep_max_length":20}, "object_types": object_types},
    ...
    if tpcc:
        ...
        dynamic_args_tpcc = {"qstag" : {"params" : {"min_rows": 1, "max_rows": 2, "max_eps": 4, "ep_max_length":20}, "object_types": object_types}}
    ...
    for counter, task in enumerate(sorted(os.listdir(directory))):
        ...
        if "196" in task: continue
        if "211" in task: continue
        # if "10" not in task: continue
        ...
        try:
            ...
            print "> %s, %s, %s, %s, %s, %s" % (counter-2, task, date, time, uuid, label)
        ...
        for cnt, f in enumerate(feature_spaces):
            ...
            if isinstance(f, sentance_featurespace): language_word = True
            ...
            for freq, hash in zip(f.histogram, f.code_book):
                ...
                try:
                    ...
                    #if it's a language word - keep the indexes in a seperate list
                    if language_word:
                        ind = len(histogram)-1
                        language_indices.append(ind)

        ids = np.nonzero(histogram)[0]
        f = open(qsr_path + "/%s.p" % task, "w")
        pickle.dump((uuid, label, ids, histogram[ids]), f, 2)
        f.close()
    ...
    f =open(qsr_path + "/language_word_indicies.p", "w")
    pickle.dump(language_indices, f, 2)
    f.close()
    pdb.set_trace()
